...
SemaphoreHandle_t xspi_rx_semaphore;
SemaphoreHandle_t xspi_tx_semaphore;
...
INT lx_stm32_ospi_lowlevel_deinit(UINT instance)
{
  ...
  vSemaphoreDelete(xspi_tx_semaphore);
  vSemaphoreDelete(xspi_rx_semaphore);
...
INT lx_stm32_ospi_write(UINT instance, ULONG *address, ULONG *buffer, ULONG words)
{
  ...
  do
  {
   ...
   /* Release xspi_transfer_semaphore in case of writing success */
    xSemaphoreGive(xspi_tx_semaphore);
...
void HAL_XSPI_RxCpltCallback(XSPI_HandleTypeDef *hxspi)
{
  ...
  xSemaphoreGiveFromISR(xspi_rx_semaphore, NULL);
...
void HAL_XSPI_TxCpltCallback(XSPI_HandleTypeDef *hxspi)
{
  ...
  xSemaphoreGiveFromISR(xspi_tx_semaphore, NULL);
