...
#define APP_DEFAULT_STACK_SIZE          2 * 1024
/* Main task priority */
...
/* Private variables ---------------------------------------------------------*/
/* Main task global data structures. */
TaskHandle_t fx_app_task;
...
void fx_app_task_entry(void * args);
...
UINT app_fileX_Init(VOID)
{
  ...
  ret = xTaskCreate(fx_app_task_entry, FX_APP_TASK_NAME, APP_DEFAULT_STACK_SIZE, (VOID *) NULL,
                    APP_TASK_PRIORITY, &fx_app_task);
...
/**
  ...
  * @param  args: user argument used by the task entry
  * @retval none
  */
void fx_app_task_entry(void * args)
{
  ...
  /* USER CODE BEGIN fx_app_task_entry 0 */
  ULONG bytes_read;
  ...
  /* USER CODE END fx_app_task_entry 0 */
  ...
  /* USER CODE BEGIN fx_app_task_entry 1 */
  ...
  /* USER CODE END fx_app_task_entry 1 */
}
