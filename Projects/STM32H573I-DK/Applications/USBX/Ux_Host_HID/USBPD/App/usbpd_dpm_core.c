...
/**
  ...
  * @retval USBPD status
  */
uint32_t USBPD_DPM_InitOS(void)
{
  OS_INIT();
  {
    OS_CREATE_QUEUE(CADQueueId, USBPD_PORT_COUNT, OS_ELEMENT_SIZE);
    OS_CREATE_TASK(USBPD_CAD_Task, CAD, OS_CAD_STACK_SIZE, OS_CAD_PRIORITY , CADThread);
  }
