...
extern EventGroupHandle_t EventFlag;
...
/**
  ...
  * @retval none
  */
VOID usbx_cdc_acm_read_thread_entry(VOID *input)
{
  ULONG actual_length;
  UX_SLAVE_DEVICE *device;
  ...
  while (1)
  {
    ...
    if ((device->ux_slave_device_state == UX_DEVICE_CONFIGURED) && (cdc_acm != UX_NULL))
    {
      ...
      if (actual_length != 0)
      {
        ...
        if (xEventGroupWaitBits(EventFlag, TX_NEW_TRANSMITTED_DATA, pdTRUE,
                               pdFALSE, portMAX_DELAY) != TX_NEW_TRANSMITTED_DATA)
        {
          Error_Handler();
        }
      }
...
/**
  ...
  * @retval none
  */
VOID usbx_cdc_acm_write_thread_entry(VOID* input)
{
  ...
  while (1)
  {
    ...
    if (xEventGroupWaitBits(EventFlag, RX_NEW_RECEIVED_DATA, pdTRUE,
                           pdFALSE, portMAX_DELAY) != RX_NEW_RECEIVED_DATA)
    {
      Error_Handler();
    }
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  xEventGroupSetBitsFromISR(EventFlag, TX_NEW_TRANSMITTED_DATA, NULL);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  xEventGroupSetBitsFromISR(EventFlag, RX_NEW_RECEIVED_DATA, NULL);
