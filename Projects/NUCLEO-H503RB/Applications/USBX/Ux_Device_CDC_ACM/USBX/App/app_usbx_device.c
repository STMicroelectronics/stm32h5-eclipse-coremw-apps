...
static TaskHandle_t ux_cdc_write_thread;
EventGroupHandle_t EventFlag;
extern PCD_HandleTypeDef           hpcd_USB_DRD_FS;
/* USER CODE END PV */

/* Private function prototypes -----------------------------------------------*/
static VOID app_ux_device_thread_entry(VOID *input);
/* USER CODE BEGIN PFP */
...
UINT MX_USBX_Device_Init(VOID)
{
  ...
  if (xTaskCreate(app_ux_device_thread_entry, "app ux device thread entry",
                       APP_DEFAULT_STACK_SIZE, (VOID *) NULL, APP_DEFAULT_PRIORITY,
                       &ux_device_app_thread) != pdPASS)
  {
  ...
  if (xTaskCreate(usbx_cdc_acm_read_thread_entry, "cdc_acm_read_usbx_app_thread_entry",
                       APP_DEFAULT_STACK_SIZE, (VOID *) NULL, APP_DEFAULT_PRIORITY,
                       &ux_cdc_read_thread) != pdPASS)
  {
    return UX_ERROR;
  }

  /* Create the usbx_cdc_acm_write_thread_entry thread */
  if (xTaskCreate(usbx_cdc_acm_write_thread_entry, "cdc_acm_write_usbx_app_thread_entry",
                       APP_DEFAULT_STACK_SIZE, (VOID *) NULL, APP_DEFAULT_PRIORITY,
                       &ux_cdc_write_thread) != pdPASS)
  {
    return UX_ERROR;
  }
